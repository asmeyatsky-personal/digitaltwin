...
async fn main() -> Result<()> {
    let cfg: Config = Figment::new().merge(Env::prefixed("CREATIVE_")).merge(Env::raw().only(&["PORT"])).extract().context("config")?;
...
async fn handle_mcp(State(mcp): State<Arc<CreativeMcp>>, Json(req): Json<JsonRpcRequest>) -> Json<JsonRpcResponse> {
