...
use learning_application::{CompleteModule, CreatePath, GetProgress, ListPaths, StartPath};
use learning_infrastructure::PostgresLearningRepository;
use learning_presentation::{
    LearningServices,
    mcp::{JsonRpcRequest, JsonRpcResponse, LearningMcp},
};
...
async fn main() -> Result<()> {
    let cfg: Config = Figment::new().merge(Env::prefixed("LEARNING_")).merge(Env::raw().only(&["PORT"])).extract().context("config")?;
    let _g = telemetry::init(telemetry::Config {
        service_name: "learning-service".into(),
    ...
    let repo = Arc::new(PostgresLearningRepository::new(pool.clone()));
    let audit = Arc::new(PostgresAuditLedger { pool });
    let clock = Arc::new(SystemClock);
    let services = LearningServices {
        create: Arc::new(CreatePath::new(repo.clone(), clock.clone())),
        list: Arc::new(ListPaths::new(repo.clone())),
        start: Arc::new(StartPath::new(repo.clone(), audit.clone(), clock.clone())),
        complete: Arc::new(CompleteModule::new(repo.clone(), audit, clock)),
        progress: Arc::new(GetProgress::new(repo)),
    };
    let mcp = Arc::new(LearningMcp::new(services.clone()));
    let http = Router::new()
        .route("/healthz", axum::routing::get(|| async { "ok" }))
        .route("/mcp", post(handle_mcp)).with_state(mcp)
        .merge(learning_presentation::router(services))
        .layer(tower_http::cors::CorsLayer::permissive());
    let addr = std::net::SocketAddr::from(([0,0,0,0], cfg.port));
    tracing::info!(%addr, "learning-service listening");
    axum::serve(tokio::net::TcpListener::bind(addr).await?, http).await?;
    Ok(())
}
async fn handle_mcp(State(mcp): State<Arc<LearningMcp>>, Json(req): Json<JsonRpcRequest>) -> Json<JsonRpcResponse> {
