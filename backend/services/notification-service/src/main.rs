...
use notification_application::{RegisterDevice, SendPush, UnregisterDevice};
use notification_infrastructure::{ExpoPushAdapter, PostgresTokenRepository};
use notification_presentation::{
    NotificationServices,
    mcp::{JsonRpcRequest, JsonRpcResponse, NotificationMcp},
};
...
async fn main() -> Result<()> {
    let cfg: Config = Figment::new()
        .merge(Env::prefixed("NOTIFICATION_"))
    ...
    let repo = Arc::new(PostgresTokenRepository::new(pool.clone()));
    let push = Arc::new(ExpoPushAdapter::new());
    let audit = Arc::new(PostgresAuditLedger { pool });
    let clock = Arc::new(SystemClock);
    let services = NotificationServices {
        register: Arc::new(RegisterDevice::new(repo.clone(), audit, clock)),
        unregister: Arc::new(UnregisterDevice::new(repo.clone())),
        send: Arc::new(SendPush::new(repo, push)),
    };
    let mcp = Arc::new(NotificationMcp::new(services.clone()));
    let http = Router::new()
        .route("/healthz", axum::routing::get(|| async { "ok" }))
        .route("/mcp", post(handle_mcp)).with_state(mcp)
        .merge(notification_presentation::router(services))
...
async fn handle_mcp(State(mcp): State<Arc<NotificationMcp>>, Json(req): Json<JsonRpcRequest>) -> Json<JsonRpcResponse> {
