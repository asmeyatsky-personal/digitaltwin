...
use community_application::{CreateGroup, CreatePost, JoinGroup, ListGroups, ListPosts};
use community_infrastructure::PostgresCommunityRepository;
use community_presentation::{
    CommunityServices,
    mcp::{CommunityMcp, JsonRpcRequest, JsonRpcResponse},
};
...
async fn main() -> Result<()> {
    let cfg: Config = Figment::new().merge(Env::prefixed("COMMUNITY_")).merge(Env::raw().only(&["PORT"])).extract().context("config")?;
    let _g = telemetry::init(telemetry::Config {
        service_name: "community-service".into(),
    ...
    let repo = Arc::new(PostgresCommunityRepository::new(pool.clone()));
    let audit = Arc::new(PostgresAuditLedger { pool });
    let clock = Arc::new(SystemClock);
    let services = CommunityServices {
        create_group: Arc::new(CreateGroup::new(repo.clone(), audit.clone(), clock.clone())),
        list_groups: Arc::new(ListGroups::new(repo.clone())),
        join: Arc::new(JoinGroup::new(repo.clone())),
        create_post: Arc::new(CreatePost::new(repo.clone(), audit, clock)),
        list_posts: Arc::new(ListPosts::new(repo)),
    };
    let mcp = Arc::new(CommunityMcp::new(services.clone()));
    let http = Router::new()
        .route("/healthz", axum::routing::get(|| async { "ok" }))
        .route("/mcp", post(handle_mcp)).with_state(mcp)
        .merge(community_presentation::router(services))
        .layer(tower_http::cors::CorsLayer::permissive());
    let addr = std::net::SocketAddr::from(([0,0,0,0], cfg.port));
    tracing::info!(%addr, "community-service listening");
    axum::serve(tokio::net::TcpListener::bind(addr).await?, http).await?;
    Ok(())
}
async fn handle_mcp(State(mcp): State<Arc<CommunityMcp>>, Json(req): Json<JsonRpcRequest>) -> Json<JsonRpcResponse> {
