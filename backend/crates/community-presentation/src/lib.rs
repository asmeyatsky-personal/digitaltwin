...
pub fn router(s: CommunityServices) -> Router {
    Router::new()
        ...
        .route("/v1/groups/{group_id}/posts", post(create_post).get(list_posts))
        .with_state(s)
}

#[derive(Deserialize)] #[serde(rename_all="snake_case")]
struct CreateGroupBody { name: String, #[serde(default)] description: String, category: String, #[serde(default="default_true")] is_moderated: bool, created_by: String }
fn default_true() -> bool { true }

async fn create_group(State(s): State<CommunityServices>, Json(b): Json<CreateGroupBody>) -> Result<Json<Value>, ApiError> {
    let created_by = EntityId::<UserRef>::from_str(&b.created_by).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad user_id".into()))?;
    let id = s.create_group.execute(CreateGroupInput {
        name: b.name, description: b.description, category: b.category,
        is_moderated: b.is_moderated, created_by,
...
#[derive(Deserialize)]
struct ListGroupsQuery { category: Option<String>, limit: Option<u32> }

async fn list_groups(State(s): State<CommunityServices>, Query(q): Query<ListGroupsQuery>) -> Result<Json<Value>, ApiError> {
    let groups = s.list_groups.execute(q.category, q.limit.unwrap_or(20))
        .await.map_err(|e| ApiError(StatusCode::SERVICE_UNAVAILABLE, e.to_string()))?;
    let arr: Vec<Value> = groups.iter().map(|g| json!({
...
#[derive(Deserialize)]
struct JoinBody { user_id: String }
async fn join(State(s): State<CommunityServices>, Path(group_id): Path<String>, Json(b): Json<JoinBody>) -> Result<StatusCode, ApiError> {
    let group_id = EntityId::from_str(&group_id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad group_id".into()))?;
    let user_id = EntityId::<UserRef>::from_str(&b.user_id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad user_id".into()))?;
    s.join.execute(group_id, user_id).await.map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
    Ok(StatusCode::NO_CONTENT)
}

#[derive(Deserialize)] #[serde(rename_all="snake_case")]
struct CreatePostBody { author: String, content: String, #[serde(default)] is_anonymous: bool }

async fn create_post(State(s): State<CommunityServices>, Path(group_id): Path<String>, Json(b): Json<CreatePostBody>) -> Result<Json<Value>, ApiError> {
    let group_id = EntityId::from_str(&group_id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad group_id".into()))?;
    let author = EntityId::<UserRef>::from_str(&b.author).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad author".into()))?;
    let id = s.create_post.execute(CreatePostInput {
        group_id, author, content: b.content, is_anonymous: b.is_anonymous,
        actor_id: EntityId::<Actor>::from_uuid(uuid::Uuid::nil()),
    }).await.map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
...
async fn list_posts(State(s): State<CommunityServices>, Path(group_id): Path<String>, Query(q): Query<LimitQuery>) -> Result<Json<Value>, ApiError> {
    let group_id = EntityId::from_str(&group_id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad group_id".into()))?;
    let posts = s.list_posts.execute(group_id, q.limit.unwrap_or(50))
        .await.map_err(|e| ApiError(StatusCode::SERVICE_UNAVAILABLE, e.to_string()))?;
...
pub mod mcp {
    ...
    pub struct CommunityMcp { s: CommunityServices }
    impl CommunityMcp {
        pub fn new(s: CommunityServices) -> Self { Self { s } }
        pub async fn handle(&self, req: JsonRpcRequest) -> JsonRpcResponse {
            ...
            match req.method.as_str() {
                ...
                "tools/call" => {
                    ...
                    let Ok(group_id) = EntityId::from_str(args.get("group_id").and_then(Value::as_str).unwrap_or("")) else {
                        return JsonRpcResponse::err(id, -32602, "bad group_id");
                    };
                    let Ok(author) = EntityId::<UserRef>::from_str(args.get("author").and_then(Value::as_str).unwrap_or("")) else {
                        return JsonRpcResponse::err(id, -32602, "bad author");
                    };
                    let content = args.get("content").and_then(Value::as_str).unwrap_or("").to_string();
                    let is_anonymous = args.get("is_anonymous").and_then(Value::as_bool).unwrap_or(false);
                    match self.s.create_post.execute(CreatePostInput {
                        group_id, author, content, is_anonymous,
                        actor_id: EntityId::<Actor>::from_uuid(uuid::Uuid::nil()),
                    }).await {
                        Ok(pid) => JsonRpcResponse::ok(id, json!({"content":[{"type":"text","text": pid.to_string()}]})),
                        Err(e) => JsonRpcResponse::err(id, -32000, e.to_string()),
                    }
                }
                "resources/read" => {
                    match self.s.list_groups.execute(None, 50).await {
                        Ok(groups) => JsonRpcResponse::ok(id, json!({
