...
impl EmotionMcp {
    #[must_use]
    pub fn new(services: EmotionServices) -> Self { Self { services } }
    ...
    async fn tool_call(&self, id: Option<Value>, params: Value) -> JsonRpcResponse {
        ...
        let Ok(modality) = Modality::parse(args.get("modality").and_then(Value::as_str).unwrap_or_default()) else {
            return JsonRpcResponse::err(id, -32602, "bad modality");
        };
        let Ok(tone) = UnifiedTone::parse(args.get("tone").and_then(Value::as_str).unwrap_or_default()) else {
            return JsonRpcResponse::err(id, -32602, "bad tone");
        };
        let confidence = args.get("confidence").and_then(Value::as_f64).unwrap_or(0.0) as f32;
        match self.services.report.execute(ReportReadingInput {
            user_id, modality, tone, confidence,
