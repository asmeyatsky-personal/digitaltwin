...
use creative_domain::{CreativeRepository, CreativeWork, DomainError, StoreError, UserRef, WorkType};
...
pub struct CreateWork { repo: Arc<dyn CreativeRepository>, audit: Arc<dyn AuditPort>, clock: Arc<dyn Clock> }
impl CreateWork {
    pub fn new(repo: Arc<dyn CreativeRepository>, audit: Arc<dyn AuditPort>, clock: Arc<dyn Clock>) -> Self { Self { repo, audit, clock } }
    pub async fn execute(&self, i: CreateWorkInput) -> Result<EntityId<CreativeWork>, UseCaseError> {
        ...
        self.audit.append(AuditEvent {
            ...
            entity_type: "CreativeWork".into(), entity_id: id.to_string(),
            before_hash: String::new(), after_hash: hash_state(&w),
        }).await?;
        Ok(id)
    }
}

pub struct ShareWork { repo: Arc<dyn CreativeRepository> }
impl ShareWork {
    pub fn new(repo: Arc<dyn CreativeRepository>) -> Self { Self { repo } }
...
pub struct GetWork { repo: Arc<dyn CreativeRepository> }
impl GetWork {
    pub fn new(repo: Arc<dyn CreativeRepository>) -> Self { Self { repo } }
...
pub struct ListWorks { repo: Arc<dyn CreativeRepository> }
impl ListWorks {
    pub fn new(repo: Arc<dyn CreativeRepository>) -> Self { Self { repo } }
    pub async fn execute(&self, i: ListWorksInput) -> Result<Vec<CreativeWork>, UseCaseError> {
        Ok(self.repo.list_for_user(i.user_id, i.work_type, i.limit).await?)
    }
}
