...
struct CreateBody { user_id: String, work_type: String, title: String, content: String, #[serde(default)] mood: String }
async fn create(State(s): State<CreativeServices>, Json(b): Json<CreateBody>) -> Result<Json<Value>, ApiError> {
    let user_id = EntityId::<UserRef>::from_str(&b.user_id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad user_id".into()))?;
    let work_type = WorkType::parse(&b.work_type).map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
    let id = s.create.execute(CreateWorkInput {
        user_id, work_type, title: b.title, content: b.content, mood: b.mood,
...
async fn get_work(State(s): State<CreativeServices>, Path(id): Path<String>) -> Result<Json<Value>, ApiError> {
    let id = EntityId::from_str(&id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad id".into()))?;
    let w = s.get.execute(id).await.map_err(|e| ApiError(StatusCode::NOT_FOUND, e.to_string()))?;
    Ok(Json(json!({
...
async fn share(State(s): State<CreativeServices>, Path(id): Path<String>) -> Result<StatusCode, ApiError> {
    let id = EntityId::from_str(&id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad id".into()))?;
    s.share.execute(id).await.map_err(|e| ApiError(StatusCode::NOT_FOUND, e.to_string()))?;
    Ok(StatusCode::NO_CONTENT)
}

#[derive(Deserialize)]
struct ListQuery { work_type: Option<String>, limit: Option<u32> }
async fn list_for_user(State(s): State<CreativeServices>, Path(user_id): Path<String>, Query(q): Query<ListQuery>) -> Result<Json<Value>, ApiError> {
    let user_id = EntityId::<UserRef>::from_str(&user_id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad user_id".into()))?;
    let work_type = q.work_type.as_deref().map(WorkType::parse).transpose()
        .map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
    let items = s.list.execute(ListWorksInput { user_id, work_type, limit: q.limit.unwrap_or(20) })
        .await.map_err(|e| ApiError(StatusCode::SERVICE_UNAVAILABLE, e.to_string()))?;
    let arr: Vec<Value> = items.iter().map(|w| json!({
        "id": w.id.to_string(), "work_type": w.work_type.as_str(),
...
pub mod mcp {
    ...
    pub struct CreativeMcp { s: CreativeServices }
    impl CreativeMcp {
        pub fn new(s: CreativeServices) -> Self { Self { s } }
        pub async fn handle(&self, req: JsonRpcRequest) -> JsonRpcResponse {
            ...
            match req.method.as_str() {
                ...
                "tools/call" => {
                    ...
                    if req.params.get("name").and_then(Value::as_str) != Some("creative.create_work") {
                    ...
                    let Ok(work_type) = WorkType::parse(args.get("work_type").and_then(Value::as_str).unwrap_or("")) else {
                    ...
                    match self.s.create.execute(CreateWorkInput {
                        user_id, work_type, title, content, mood,
                        actor_id: EntityId::<Actor>::from_uuid(uuid::Uuid::nil()),
                    }).await {
                        Ok(id2) => JsonRpcResponse::ok(id, json!({"content":[{"type":"text","text": id2.to_string()}]})),
