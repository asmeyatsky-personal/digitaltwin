...
use avatar_domain::{GenerationJob, GeneratorError, GeneratorPort, JobRepository, JobStatus, StoreError, UserRef};
...
impl PostgresJobRepository {
    ...
    fn row_to_job(r: PgRow) -> Result<GenerationJob, StoreError> {
        ...
        let photo_url: String = r.try_get("photo_url").map_err(|e| StoreError::Backend(e.to_string()))?;
        let status: String = r.try_get("status").map_err(|e| StoreError::Backend(e.to_string()))?;
        let result_url: Option<String> = r.try_get("result_url").map_err(|e| StoreError::Backend(e.to_string()))?;
        let failure_reason: Option<String> = r.try_get("failure_reason").map_err(|e| StoreError::Backend(e.to_string()))?;
        ...
        let status = JobStatus::parse(&status).ok_or_else(|| StoreError::Backend(format!("bad status: {status}")))?;
        Ok(GenerationJob {
            id: EntityId::from_uuid(id), user_id: EntityId::from_uuid(user_id),
            photo_url, status, result_url, failure_reason, created_at, completed_at,
        })
    }
}
...
impl GeneratorPort for PythonAvatarAdapter {
    async fn generate(&self, job_id: EntityId<GenerationJob>, photo_url: &str) -> Result<String, GeneratorError> {
        ...
        let resp = tokio::time::timeout(self.timeout, self.client.post(&url).json(&payload).send())
            .await.map_err(|_| GeneratorError::Timeout)?
            .map_err(|e| GeneratorError::Failed(e.to_string()))?;
        if !resp.status().is_success() {
            return Err(GeneratorError::Failed(format!("http {}", resp.status())));
        }
        let body: serde_json::Value = resp.json().await.map_err(|e| GeneratorError::Failed(e.to_string()))?;
        body.get("result_url").and_then(|v| v.as_str()).map(str::to_string)
