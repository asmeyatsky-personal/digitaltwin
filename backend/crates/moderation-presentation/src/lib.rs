...
use kernel::EntityId;
use moderation_application::{
    ListPending, ReportContent, ReportContentInput, ReviewReport, ReviewReportInput,
};
...
async fn report(
    State(s): State<ModerationServices>,
    Json(b): Json<ReportBody>,
) -> Result<Json<Value>, ApiError> {
    let reporter = EntityId::<UserRef>::from_str(&b.reporter)
        .map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad reporter".into()))?;
    let reason =
        Reason::parse(&b.reason).map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
    let id = s
        .report
        .execute(ReportContentInput {
            reporter,
...
async fn review(
    ...
    let report_id = EntityId::from_str(&report_id)
        .map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad report_id".into()))?;
    let reviewer = EntityId::<UserRef>::from_str(&b.reviewer)
        .map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad reviewer".into()))?;
    let status =
        Status::parse(&b.status).map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
    s.review
        .execute(ReviewReportInput {
            report_id,
            reviewer,
            status,
            notes: b.notes,
...
pub mod mcp {
    ...
    pub struct ModerationMcp {
        s: ModerationServices,
    }
    impl ModerationMcp {
        pub fn new(s: ModerationServices) -> Self {
            Self { s }
        }
        pub async fn handle(&self, req: JsonRpcRequest) -> JsonRpcResponse {
            ...
            match req.method.as_str() {
                ...
                "tools/call" => {
                    ...
                    let Ok(reporter) = EntityId::<UserRef>::from_str(
                        args.get("reporter").and_then(Value::as_str).unwrap_or(""),
                    ) else {
                    ...
                    let content_id = args
                        ...
                        .to_string();
                    let Ok(reason) =
                        Reason::parse(args.get("reason").and_then(Value::as_str).unwrap_or(""))
                    else {
                    ...
                    match self
                        ...
                        .await
                    {
                        Ok(rid) => JsonRpcResponse::ok(
                            id,
                            json!({"content":[{"type":"text","text": rid.to_string()}]}),
                        ),
                        Err(e) => JsonRpcResponse::err(id, -32000, e.to_string()),
                    }
                }
                "resources/read" => match self.s.pending.execute(50).await {
