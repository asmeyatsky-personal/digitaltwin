...
pub fn router(s: FamilyServices) -> Router {
    Router::new()
        ...
        .route("/v1/families/{family_id}/members", post(add_member).get(list_members))
        .route("/v1/users/{user_id}/families", get(list_for_user))
        .with_state(s)
}

#[derive(Deserialize)] #[serde(rename_all="snake_case")]
struct CreateBody { name: String, created_by_user_id: String }

async fn create(State(s): State<FamilyServices>, Json(b): Json<CreateBody>) -> Result<Json<Value>, ApiError> {
    let created_by = EntityId::<UserRef>::from_str(&b.created_by_user_id)
        .map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad user_id".into()))?;
    let id = s.create.execute(CreateFamilyInput {
        name: b.name, created_by, actor_id: EntityId::<Actor>::from_uuid(uuid::Uuid::nil()),
    }).await.map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
    Ok(Json(json!({ "family_id": id.to_string() })))
}

#[derive(Deserialize)] #[serde(rename_all="snake_case")]
struct AddMemberBody { user_id: String, role: String }
...
async fn add_member(
    ...
    let family_id = EntityId::from_str(&family_id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad family_id".into()))?;
    let user_id = EntityId::<UserRef>::from_str(&b.user_id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad user_id".into()))?;
    let role = FamilyRole::parse(&b.role).map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
    s.add_member.execute(AddMemberInput {
        family_id, user_id, role, actor_id: EntityId::<Actor>::from_uuid(uuid::Uuid::nil()),
    }).await.map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
    Ok(StatusCode::NO_CONTENT)
}

async fn get_family(State(s): State<FamilyServices>, Path(id): Path<String>) -> Result<Json<Value>, ApiError> {
    let id = EntityId::from_str(&id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad id".into()))?;
    let f = s.get.execute(id).await.map_err(|e| ApiError(StatusCode::NOT_FOUND, e.to_string()))?;
    Ok(Json(json!({
...
async fn list_members(State(s): State<FamilyServices>, Path(id): Path<String>) -> Result<Json<Value>, ApiError> {
    let id = EntityId::from_str(&id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad id".into()))?;
    let members = s.list_members.execute(id).await.map_err(|e| ApiError(StatusCode::NOT_FOUND, e.to_string()))?;
    let items: Vec<Value> = members.iter().map(|m| json!({
...
async fn list_for_user(State(s): State<FamilyServices>, Path(user_id): Path<String>) -> Result<Json<Value>, ApiError> {
    let user_id = EntityId::<UserRef>::from_str(&user_id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad user_id".into()))?;
    let families = s.list_for_user.execute(user_id).await.map_err(|e| ApiError(StatusCode::SERVICE_UNAVAILABLE, e.to_string()))?;
    let items: Vec<Value> = families.iter().map(|f| json!({
        "id": f.id.to_string(), "name": f.name,
        "created_by": f.created_by.to_string(), "created_at": f.created_at.to_rfc3339(),
    })).collect();
...
pub mod mcp {
    ...
    pub struct FamilyMcp { s: FamilyServices }
    impl FamilyMcp {
        pub fn new(s: FamilyServices) -> Self { Self { s } }
        pub async fn handle(&self, req: JsonRpcRequest) -> JsonRpcResponse {
            ...
            match req.method.as_str() {
                ...
                "tools/call" => {
                    ...
                    match name {
                        "family.create" => {
                            let name_s = args.get("name").and_then(Value::as_str).unwrap_or("").to_string();
                            let Ok(created_by) = EntityId::<UserRef>::from_str(args.get("created_by").and_then(Value::as_str).unwrap_or("")) else {
                                return JsonRpcResponse::err(id, -32602, "bad created_by");
                            };
                            match self.s.create.execute(CreateFamilyInput { name: name_s, created_by, actor_id: EntityId::<Actor>::from_uuid(uuid::Uuid::nil()) }).await {
                                Ok(fid) => JsonRpcResponse::ok(id, json!({"content": [{"type":"text","text": fid.to_string()}]})),
                                Err(e) => JsonRpcResponse::err(id, -32000, e.to_string()),
                            }
                        }
                        "family.add_member" => {
                            let Ok(family_id) = EntityId::from_str(args.get("family_id").and_then(Value::as_str).unwrap_or("")) else {
                            ...
                            let Ok(role) = FamilyRole::parse(args.get("role").and_then(Value::as_str).unwrap_or("")) else {
                                return JsonRpcResponse::err(id, -32602, "bad role");
                            };
                            match self.s.add_member.execute(AddMemberInput { family_id, user_id, role, actor_id: EntityId::<Actor>::from_uuid(uuid::Uuid::nil()) }).await {
                                Ok(()) => JsonRpcResponse::ok(id, json!({"content":[{"type":"text","text":"added"}]})),
                                Err(e) => JsonRpcResponse::err(id, -32000, e.to_string()),
                            }
                        }
                ...
                "resources/read" => {
                    ...
                    match self.s.list_members.execute(fid).await {
                        Ok(members) => JsonRpcResponse::ok(id, json!({
                            "contents": [{
                                "uri": uri,
