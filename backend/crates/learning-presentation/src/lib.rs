...
async fn create_path(
    State(s): State<LearningServices>,
    Json(b): Json<CreateBody>,
) -> Result<Json<Value>, ApiError> {
    let id = s
        .create
        .execute(CreatePathInput {
            title: b.title,
...
async fn start(
    ...
    let path_id = EntityId::from_str(&path_id)
        .map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad path_id".into()))?;
    let user_id = EntityId::<UserRef>::from_str(&b.user_id)
        .map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad user_id".into()))?;
    s.start
        .execute(StartPathInput {
            user_id,
            path_id,
...
async fn complete(
    ...
    let completed = s
        .complete
        .execute(CompleteModuleInput {
            user_id,
            path_id,
            reflection_notes: b.reflection_notes,
            actor_id: EntityId::<Actor>::from_uuid(uuid::Uuid::nil()),
        })
        .await
        .map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
...
pub mod mcp {
    ...
    impl LearningMcp {
        pub fn new(s: LearningServices) -> Self {
            Self { s }
        }
        pub async fn handle(&self, req: JsonRpcRequest) -> JsonRpcResponse {
            ...
            match req.method.as_str() {
                ...
                "tools/call" => {
                    ...
                    let Ok(path_id) = EntityId::from_str(
                        args.get("path_id").and_then(Value::as_str).unwrap_or(""),
                    ) else {
                        return JsonRpcResponse::err(id, -32602, "bad path_id");
                    };
                    let notes = args
                        .get("reflection_notes")
                    ...
                    match self
                        ...
                        .await
                    {
                        Ok(done) => JsonRpcResponse::ok(
                            id,
                            json!({"content":[{"type":"text","text": if done {"path complete"} else {"module advanced"}}]}),
                        ),
                        Err(e) => JsonRpcResponse::err(id, -32000, e.to_string()),
                    }
                }
                "resources/read" => match self.s.list.execute(None).await {
                    Ok(paths) => JsonRpcResponse::ok(
                        id,
                        json!({
