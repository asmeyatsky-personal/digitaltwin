...
pub struct RegisterTherapist {
    repo: Arc<dyn TherapyRepository>,
...
impl RegisterTherapist {
    ...
    pub async fn execute(
        &self,
        i: RegisterTherapistInput,
    ) -> Result<EntityId<TherapistProfile>, UseCaseError> {
        let now = self.clock.now();
        let id = EntityId::<TherapistProfile>::new();
        let t = TherapistProfile::new(
            id,
            i.name,
            i.credentials,
            i.specializations,
        ...
        self.audit
            .append(AuditEvent {
                ...
                entity_type: "TherapistProfile".into(),
                entity_id: id.to_string(),
                before_hash: String::new(),
                after_hash: hash_state(&t),
            })
            .await?;
        Ok(id)
    }
...
pub struct ListTherapists {
    repo: Arc<dyn TherapyRepository>,
}
impl ListTherapists {
    pub fn new(repo: Arc<dyn TherapyRepository>) -> Self {
        Self { repo }
    }
...
impl SubmitScreening {
    ...
    pub async fn execute(
        &self,
        i: SubmitScreeningInput,
    ) -> Result<SubmitScreeningOutput, UseCaseError> {
        let now = self.clock.now();
        ...
        self.audit
            .append(AuditEvent {
                ...
                entity_type: "ClinicalScreening".into(),
                entity_id: id.to_string(),
                before_hash: String::new(),
                after_hash: hash_state(&s),
            })
            .await?;
        Ok(SubmitScreeningOutput {
            screening_id: id,
            score: s.score,
            severity: s.severity,
        })
...
pub struct ListScreeningsForUser {
    repo: Arc<dyn TherapyRepository>,
}
impl ListScreeningsForUser {
    ...
    pub async fn execute(
        ...
        limit: u32,
    ) -> Result<Vec<ClinicalScreening>, UseCaseError> {
