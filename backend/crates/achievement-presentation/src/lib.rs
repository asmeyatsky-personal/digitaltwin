...
async fn list_all(State(s): State<AchievementServices>) -> Result<Json<Value>, ApiError> {
    let items = s.list_all.execute().await.map_err(|e| ApiError(StatusCode::SERVICE_UNAVAILABLE, e.to_string()))?;
...
struct RecordBody { user_id: String, achievement_key: String, delta: u32 }

async fn record(State(s): State<AchievementServices>, Json(b): Json<RecordBody>) -> Result<Json<Value>, ApiError> {
    let user_id = EntityId::<UserRef>::from_str(&b.user_id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad user_id".into()))?;
    let out = s.record.execute(RecordProgressInput {
        user_id, achievement_key: b.achievement_key, delta: b.delta,
        actor_id: EntityId::<Actor>::from_uuid(uuid::Uuid::nil()),
    }).await.map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
    Ok(Json(json!({ "progress": out.progress, "unlocked": out.unlocked })))
}

async fn list_for_user(State(s): State<AchievementServices>, Path(user_id): Path<String>) -> Result<Json<Value>, ApiError> {
    let user_id = EntityId::<UserRef>::from_str(&user_id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad user_id".into()))?;
    let items = s.list_for_user.execute(user_id).await.map_err(|e| ApiError(StatusCode::SERVICE_UNAVAILABLE, e.to_string()))?;
    let arr: Vec<Value> = items.iter().map(|u| json!({
...
pub mod mcp {
    ...
    pub struct AchievementMcp { s: AchievementServices }
    impl AchievementMcp {
        pub fn new(s: AchievementServices) -> Self { Self { s } }
        pub async fn handle(&self, req: JsonRpcRequest) -> JsonRpcResponse {
            ...
            match req.method.as_str() {
                ...
                "tools/call" => {
                    ...
                    let achievement_key = args.get("achievement_key").and_then(Value::as_str).unwrap_or("").to_string();
                    let delta = args.get("delta").and_then(Value::as_u64).unwrap_or(1) as u32;
                    match self.s.record.execute(RecordProgressInput {
                        user_id, achievement_key, delta,
                        actor_id: EntityId::<Actor>::from_uuid(uuid::Uuid::nil()),
                    }).await {
                ...
                "resources/read" => {
                    ...
                    match self.s.list_for_user.execute(user_id).await {
                        Ok(items) => JsonRpcResponse::ok(id, json!({
