...
use learning_domain::{
    DomainError, LearningPath, LearningRepository, StoreError, UserProgress, UserRef,
};
use std::sync::Arc;
use thiserror::Error;
...
impl CreatePath {
    pub fn new(repo: Arc<dyn LearningRepository>, clock: Arc<dyn Clock>) -> Self {
        Self { repo, clock }
    }
    pub async fn execute(
        &self,
        i: CreatePathInput,
    ) -> Result<EntityId<LearningPath>, UseCaseError> {
        let id = EntityId::<LearningPath>::new();
        let p = LearningPath::new(
            id,
            i.title,
            i.description,
            i.category,
...
impl ListPaths {
    ...
    ) -> Result<Vec<LearningPath>, UseCaseError> {
...
impl CompleteModule {
    ...
    pub async fn execute(&self, i: CompleteModuleInput) -> Result<bool, UseCaseError> {
        ...
        let existing = self
            .repo
            .get_progress(i.user_id, i.path_id)
            .await?
            .ok_or(UseCaseError::NotFound)?;
        let now = self.clock.now();
        ...
        self.audit
            .append(AuditEvent {
                ...
                after_hash: hash_state(&advanced),
            })
            .await?;
...
impl GetProgress {
    ...
    ) -> Result<Vec<UserProgress>, UseCaseError> {
