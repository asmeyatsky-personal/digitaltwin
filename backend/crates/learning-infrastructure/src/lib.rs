...
pub struct PostgresLearningRepository { pool: PgPool }
impl PostgresLearningRepository {
    ...
    fn row_to_path(r: PgRow) -> Result<LearningPath, StoreError> {
        ...
        let modules_json: String = r.try_get("modules").map_err(|e| StoreError::Backend(e.to_string()))?;
        let estimated_minutes: i32 = r.try_get("estimated_minutes").map_err(|e| StoreError::Backend(e.to_string()))?;
        ...
        LearningPath::new(EntityId::from_uuid(id), title, description, category, modules, estimated_minutes as u32, created_at)
    ...
    fn row_to_progress(r: PgRow) -> Result<UserProgress, StoreError> {
        ...
        let path_id: Uuid = r.try_get("path_id").map_err(|e| StoreError::Backend(e.to_string()))?;
        let current_module: i32 = r.try_get("current_module").map_err(|e| StoreError::Backend(e.to_string()))?;
        let reflection_notes: String = r.try_get("reflection_notes").map_err(|e| StoreError::Backend(e.to_string()))?;
        let started_at: DateTime<Utc> = r.try_get("started_at").map_err(|e| StoreError::Backend(e.to_string()))?;
        let completed_at: Option<DateTime<Utc>> = r.try_get("completed_at").map_err(|e| StoreError::Backend(e.to_string()))?;
        Ok(UserProgress {
            user_id: EntityId::from_uuid(user_id), path_id: EntityId::from_uuid(path_id),
            current_module: current_module as u32, reflection_notes, started_at, completed_at,
        })
    }
}
