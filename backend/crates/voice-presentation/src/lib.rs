...
struct RegBody { user_id: String, sample_url: String }
async fn register(State(s): State<VoiceServices>, Json(b): Json<RegBody>) -> Result<StatusCode, ApiError> {
    let user_id = EntityId::<UserRef>::from_str(&b.user_id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad user_id".into()))?;
    s.register.execute(RegisterVoiceInput {
        user_id, sample_url: b.sample_url, actor_id: EntityId::<Actor>::from_uuid(uuid::Uuid::nil()),
    }).await.map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
    Ok(StatusCode::NO_CONTENT)
}

#[derive(Deserialize)] #[serde(rename_all="snake_case")]
struct SynthBody { user_id: String, text: String, #[serde(default="default_emotion")] emotion: String }
fn default_emotion() -> String { "neutral".into() }
async fn synthesize(State(s): State<VoiceServices>, Json(b): Json<SynthBody>) -> Result<Json<Value>, ApiError> {
    let user_id = EntityId::<UserRef>::from_str(&b.user_id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad user_id".into()))?;
    let out = s.synthesize.execute(SynthesizeInput { user_id, text: b.text, emotion: b.emotion })
        .await.map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
    Ok(Json(json!({ "job_id": out.job_id.to_string(), "audio_url": out.audio_url })))
}

async fn get_job(State(s): State<VoiceServices>, Path(id): Path<String>) -> Result<Json<Value>, ApiError> {
    let id = EntityId::from_str(&id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad id".into()))?;
    let j = s.get_job.execute(id).await.map_err(|e| ApiError(StatusCode::NOT_FOUND, e.to_string()))?;
    Ok(Json(json!({
...
pub mod mcp {
    ...
    pub struct VoiceMcp { s: VoiceServices }
    impl VoiceMcp {
        pub fn new(s: VoiceServices) -> Self { Self { s } }
        pub async fn handle(&self, req: JsonRpcRequest) -> JsonRpcResponse {
            ...
            match req.method.as_str() {
                ...
                "tools/call" => {
                    ...
                    let emotion = args.get("emotion").and_then(Value::as_str).unwrap_or("neutral").to_string();
                    match self.s.synthesize.execute(SynthesizeInput { user_id, text, emotion }).await {
