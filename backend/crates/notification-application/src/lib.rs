...
use notification_domain::{
    DeviceToken, DomainError, Payload, Platform, PushError, PushPort, StoreError, TokenRepository,
    UserRef,
};
use std::sync::Arc;
use thiserror::Error;
...
impl SendPush {
    pub fn new(repo: Arc<dyn TokenRepository>, push: Arc<dyn PushPort>) -> Self {
        Self { repo, push }
    }
