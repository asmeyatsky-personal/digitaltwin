...
use therapy_domain::{
    ClinicalScreening, ScreeningType, Severity, StoreError, TherapistProfile, TherapyRepository,
    UserRef,
};
use uuid::Uuid;

pub struct PostgresTherapyRepository {
    pool: PgPool,
}
impl PostgresTherapyRepository {
    ...
    fn row_to_therapist(r: PgRow) -> Result<TherapistProfile, StoreError> {
        ...
        let name: String = r
            .try_get("name")
            .map_err(|e| StoreError::Backend(e.to_string()))?;
        let credentials: String = r
            .try_get("credentials")
            .map_err(|e| StoreError::Backend(e.to_string()))?;
        let specializations_json: String = r
            .try_get("specializations")
            .map_err(|e| StoreError::Backend(e.to_string()))?;
        let rate_per_session: i32 = r
            .try_get("rate_per_session")
            .map_err(|e| StoreError::Backend(e.to_string()))?;
        let is_verified: bool = r
            .try_get("is_verified")
            .map_err(|e| StoreError::Backend(e.to_string()))?;
        ...
        let specializations: Vec<String> =
            serde_json::from_str(&specializations_json).unwrap_or_default();
        let t = TherapistProfile::new(
            EntityId::from_uuid(id),
            name,
            credentials,
            specializations,
            rate_per_session as u32,
            created_at,
        )
        .map_err(|e| StoreError::Backend(e.to_string()))?;
    ...
    fn row_to_screening(r: PgRow) -> Result<ClinicalScreening, StoreError> {
        ...
        let user_id: Uuid = r
            .try_get("user_id")
            .map_err(|e| StoreError::Backend(e.to_string()))?;
        let screening_type: String = r
            .try_get("screening_type")
            .map_err(|e| StoreError::Backend(e.to_string()))?;
        let responses_json: String = r
            .try_get("responses")
            .map_err(|e| StoreError::Backend(e.to_string()))?;
        let score: i32 = r
            .try_get("score")
        ...
        let severity: String = r
            .try_get("severity")
            .map_err(|e| StoreError::Backend(e.to_string()))?;
        let completed_at: DateTime<Utc> = r
        ...
        Ok(ClinicalScreening {
            ...
            screening_type: ScreeningType::parse(&screening_type)
                .map_err(|e| StoreError::Backend(e.to_string()))?,
            responses,
            score: score as u32,
