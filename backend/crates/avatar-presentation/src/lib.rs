...
async fn list_for_user(
    ...
    Ok(Json(
        json!({ "jobs": jobs.iter().map(job_to_json).collect::<Vec<_>>() }),
    ))
}
...
pub mod mcp {
    ...
    pub struct AvatarMcp {
        s: AvatarServices,
    }
    impl AvatarMcp {
        pub fn new(s: AvatarServices) -> Self {
            Self { s }
        }
            ...
            match req.method.as_str() {
                ...
                "tools/call" => {
                    ...
                    match self
                        ...
                        .await
                    {
                        Ok(jid) => JsonRpcResponse::ok(
                            id,
                            json!({"content":[{"type":"text","text": jid.to_string()}]}),
                        ),
