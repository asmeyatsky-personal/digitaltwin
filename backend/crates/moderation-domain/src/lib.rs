...
impl ContentReport {
    pub fn submit(
        id: EntityId<ContentReport>,
        reporter: EntityId<UserRef>,
        content_type: String,
        content_id: String,
        reason: Reason,
        now: DateTime<Utc>,
        ...
        if content_id.trim().is_empty() {
            return Err(DomainError::EmptyContentId);
        }
        Ok(Self {
            id,
            reporter,
...
mod tests {
    ...
    fn rejects_empty_content_id() {
        assert_eq!(
            ...
            .unwrap_err(),
            DomainError::EmptyContentId
        );
    }
    #[test]
    fn review_transitions_status() {
        let r = ContentReport::submit(
            EntityId::new(),
            EntityId::new(),
            "post".into(),
            "abc".into(),
