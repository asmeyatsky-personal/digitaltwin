...
pub struct PostgresFamilyRepository { pool: PgPool }
impl PostgresFamilyRepository {
    ...
    fn row_to_family(r: PgRow) -> Result<Family, StoreError> {
        ...
        Family::new(EntityId::from_uuid(id), name, EntityId::from_uuid(created_by), created_at)
            .map_err(|e| StoreError::Backend(e.to_string()))
    }
    fn row_to_member(r: PgRow) -> Result<FamilyMember, StoreError> {
        let family_id: Uuid = r.try_get("family_id").map_err(|e| StoreError::Backend(e.to_string()))?;
        let user_id: Uuid = r.try_get("user_id").map_err(|e| StoreError::Backend(e.to_string()))?;
        let role: String = r.try_get("role").map_err(|e| StoreError::Backend(e.to_string()))?;
        let joined_at: DateTime<Utc> = r.try_get("joined_at").map_err(|e| StoreError::Backend(e.to_string()))?;
        Ok(FamilyMember {
...
impl FamilyRepository for PostgresFamilyRepository {
    async fn insert_family(&self, f: &Family) -> Result<(), StoreError> {
        sqlx::query("INSERT INTO family.families (id, name, created_by, created_at) VALUES ($1,$2,$3,$4)")
            .bind(f.id.as_uuid()).bind(&f.name).bind(f.created_by.as_uuid()).bind(f.created_at)
    ...
    async fn get_family(&self, id: EntityId<Family>) -> Result<Option<Family>, StoreError> {
        let row = sqlx::query("SELECT id, name, created_by, created_at FROM family.families WHERE id=$1")
    ...
    async fn add_member(&self, m: &FamilyMember) -> Result<(), StoreError> {
        ...
        .bind(m.family_id.as_uuid()).bind(m.user_id.as_uuid())
        .bind(m.role.as_str()).bind(m.joined_at)
        .execute(&self.pool).await.map_err(|e| StoreError::Backend(e.to_string()))?;
        Ok(())
    }
