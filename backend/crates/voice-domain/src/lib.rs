...
pub enum DomainError {
    ...
    EmptyText,
    #[error("sample_url cannot be empty")]
    EmptySample,
}
...
impl VoiceProfile {
    ...
    pub fn with_clone(&self, cloned_voice_id: String) -> Self {
        Self {
            cloned_voice_id: Some(cloned_voice_id),
            ..self.clone()
        }
    }
}
...
pub trait VoiceRepository: Send + Sync {
    ...
    async fn get_job(&self, id: EntityId<SynthesisJob>)
...
mod tests {
    use super::*;
    #[test]
    fn profile_rejects_empty_sample() {
        assert_eq!(
            VoiceProfile::register(EntityId::new(), "".into(), Utc::now()).unwrap_err(),
            DomainError::EmptySample
        );
    }
    #[test]
    fn job_rejects_empty_text() {
        assert_eq!(
            SynthesisJob::submit(
                EntityId::new(),
                EntityId::new(),
