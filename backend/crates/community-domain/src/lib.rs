...
impl CommunityGroup {
    pub fn new(id: EntityId<CommunityGroup>, name: String, description: String, category: String, is_moderated: bool, created_by: EntityId<UserRef>, now: DateTime<Utc>) -> Result<Self, DomainError> {
        if name.trim().is_empty() { return Err(DomainError::EmptyName); }
        Ok(Self { id, name, description, category, is_moderated, created_by, created_at: now })
    }
}
...
impl CommunityPost {
    pub fn new(id: EntityId<CommunityPost>, group_id: EntityId<CommunityGroup>, author: EntityId<UserRef>, content: String, is_anonymous: bool, now: DateTime<Utc>) -> Result<Self, DomainError> {
        if content.trim().is_empty() { return Err(DomainError::EmptyContent); }
        Ok(Self { id, group_id, author, content, is_anonymous, created_at: now })
    }
}
...
mod tests {
    ...
    #[test] fn rejects_empty_name() {
        assert_eq!(CommunityGroup::new(EntityId::new(), "".into(), "".into(), "support".into(), false, EntityId::new(), Utc::now()).unwrap_err(), DomainError::EmptyName);
    }
    #[test] fn rejects_empty_content() {
        assert_eq!(CommunityPost::new(EntityId::new(), EntityId::new(), EntityId::new(), "".into(), false, Utc::now()).unwrap_err(), DomainError::EmptyContent);
    }
}
