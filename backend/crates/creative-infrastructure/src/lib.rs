...
pub struct PostgresCreativeRepository {
    pool: PgPool,
}
...
impl CreativeRepository for PostgresCreativeRepository {
    ...
    async fn update(&self, w: &CreativeWork) -> Result<(), StoreError> {
        sqlx::query(
            "UPDATE creative.works SET title=$2, content=$3, mood=$4, is_shared=$5 WHERE id=$1",
        )
        .bind(w.id.as_uuid())
        .bind(&w.title)
        .bind(&w.content)
