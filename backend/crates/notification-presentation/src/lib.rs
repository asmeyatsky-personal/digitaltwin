...
struct RegisterBody { user_id: String, token: String, platform: String }

async fn register(State(s): State<NotificationServices>, Json(b): Json<RegisterBody>) -> Result<StatusCode, ApiError> {
    let user_id = EntityId::<UserRef>::from_str(&b.user_id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad user_id".into()))?;
    let platform = Platform::parse(&b.platform).map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
    s.register.execute(RegisterDeviceInput {
        user_id, token: b.token, platform,
        actor_id: EntityId::<Actor>::from_uuid(uuid::Uuid::nil()),
    }).await.map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
    Ok(StatusCode::NO_CONTENT)
}

async fn unregister(State(s): State<NotificationServices>, axum::extract::Path(token): axum::extract::Path<String>) -> Result<StatusCode, ApiError> {
    s.unregister.execute(&token).await.map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
    Ok(StatusCode::NO_CONTENT)
}

#[derive(Deserialize)] #[serde(rename_all="snake_case")]
struct PushBody { user_id: String, title: String, body: String, #[serde(default)] data: Value }

async fn send(State(s): State<NotificationServices>, Json(b): Json<PushBody>) -> Result<Json<Value>, ApiError> {
    let user_id = EntityId::<UserRef>::from_str(&b.user_id).map_err(|_| ApiError(StatusCode::BAD_REQUEST, "bad user_id".into()))?;
    let n = s.send.execute(SendPushInput {
        user_id, title: b.title, body: b.body, data: b.data,
...
pub mod mcp {
    ...
    pub struct NotificationMcp { s: NotificationServices }
    impl NotificationMcp {
        pub fn new(s: NotificationServices) -> Self { Self { s } }
        pub async fn handle(&self, req: JsonRpcRequest) -> JsonRpcResponse {
            ...
            match req.method.as_str() {
                ...
                "tools/call" => {
                    ...
                    match self.s.send.execute(SendPushInput { user_id, title, body, data }).await {
                        Ok(n) => JsonRpcResponse::ok(id, json!({"content":[{"type":"text","text":format!("delivered {n}")}]})),
