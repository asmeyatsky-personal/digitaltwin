...
use avatar_domain::{
    DomainError, GenerationJob, GeneratorError, GeneratorPort, JobRepository, StoreError, UserRef,
};
...
pub struct GenerateAvatarInput {
...
impl GenerateAvatar {
    ...
    pub async fn execute(
        &self,
        i: GenerateAvatarInput,
    ) -> Result<EntityId<GenerationJob>, UseCaseError> {
        ...
        self.audit
            .append(AuditEvent {
                ...
                entity_type: "GenerationJob".into(),
                entity_id: id.to_string(),
                before_hash: String::new(),
                after_hash: hash_state(&final_job),
            })
            .await?;
        Ok(id)
    }
...
impl GetJob {
    ...
    pub async fn execute(
        &self,
        id: EntityId<GenerationJob>,
    ) -> Result<GenerationJob, UseCaseError> {
        self.repo.get(id).await?.ok_or(UseCaseError::NotFound)
    }
}

pub struct ListJobsForUser {
    repo: Arc<dyn JobRepository>,
}
impl ListJobsForUser {
    ...
    ) -> Result<Vec<GenerationJob>, UseCaseError> {
