...
pub enum DomainError {
    ...
    #[error("unknown screening type: {0}")] UnknownScreening(String),
    #[error("invalid PHQ-9 response: expected {expected} items with scores 0-3")] BadPhq9 { expected: u32 },
    #[error("invalid GAD-7 response: expected {expected} items with scores 0-3")] BadGad7 { expected: u32 },
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[serde(rename_all = "UPPERCASE")]
pub enum ScreeningType { Phq9, Gad7 }
impl ScreeningType {
    ...
    pub fn as_str(self) -> &'static str { match self { Self::Phq9 => "PHQ9", Self::Gad7 => "GAD7" } }
    pub fn expected_items(self) -> u32 { match self { Self::Phq9 => 9, Self::Gad7 => 7 } }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum Severity { None, Mild, Moderate, ModeratelySevere, Severe }
impl Severity {
    pub fn as_str(self) -> &'static str {
        match self { Self::None=>"none",Self::Mild=>"mild",Self::Moderate=>"moderate",Self::ModeratelySevere=>"moderately_severe",Self::Severe=>"severe" }
    }
}
...
impl TherapistProfile {
    pub fn new(id: EntityId<TherapistProfile>, name: String, credentials: String, specializations: Vec<String>, rate_per_session: u32, now: DateTime<Utc>) -> Result<Self, DomainError> {
        if name.trim().is_empty() { return Err(DomainError::EmptyName); }
        Ok(Self { id, name, credentials, specializations, rate_per_session, is_verified: false, created_at: now })
    }
    pub fn verify(&self) -> Self { Self { is_verified: true, ..self.clone() } }
}
...
impl ClinicalScreening {
    pub fn submit(id: EntityId<ClinicalScreening>, user_id: EntityId<UserRef>, screening_type: ScreeningType, responses: Vec<u8>, now: DateTime<Utc>) -> Result<Self, DomainError> {
        if u32::try_from(responses.len()).unwrap_or(u32::MAX) != screening_type.expected_items() || responses.iter().any(|r| *r > 3) {
        ...
        Ok(Self { id, user_id, screening_type, responses, score, severity, completed_at: now })
    }
}
...
mod tests {
    use super::*;
    #[test] fn phq9_severe_threshold() {
        let s = ClinicalScreening::submit(EntityId::new(), EntityId::new(), ScreeningType::Phq9, vec![3,3,3,3,3,3,3,1,1], Utc::now()).unwrap();
        assert_eq!(s.score, 23);
        assert_eq!(s.severity, Severity::Severe);
    }
    #[test] fn gad7_minimal() {
        let s = ClinicalScreening::submit(EntityId::new(), EntityId::new(), ScreeningType::Gad7, vec![0,1,0,1,0,0,1], Utc::now()).unwrap();
        assert_eq!(s.score, 3);
        assert_eq!(s.severity, Severity::None);
    }
    #[test] fn rejects_wrong_item_count() {
        assert!(matches!(
            ClinicalScreening::submit(EntityId::new(), EntityId::new(), ScreeningType::Phq9, vec![0; 8], Utc::now()),
            Err(DomainError::BadPhq9 { expected: 9 })
        ));
    }
    #[test] fn rejects_out_of_range_responses() {
        assert!(ClinicalScreening::submit(EntityId::new(), EntityId::new(), ScreeningType::Gad7, vec![4,0,0,0,0,0,0], Utc::now()).is_err());
    }
}
