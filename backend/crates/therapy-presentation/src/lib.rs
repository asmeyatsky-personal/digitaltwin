...
async fn register(
    ...
    let id = s
        ...
        .execute(RegisterTherapistInput {
            ...
            rate_per_session: b.rate_per_session,
...
async fn submit(
    ...
    let screening_type = ScreeningType::parse(&b.screening_type)
        .map_err(|e| ApiError(StatusCode::BAD_REQUEST, e.to_string()))?;
    let out = s
        .submit
        .execute(SubmitScreeningInput {
            user_id,
            screening_type,
...
pub mod mcp {
    ...
    pub struct TherapyMcp {
        s: TherapyServices,
    }
    impl TherapyMcp {
        pub fn new(s: TherapyServices) -> Self {
            Self { s }
        }
        pub async fn handle(&self, req: JsonRpcRequest) -> JsonRpcResponse {
            ...
            match req.method.as_str() {
                ...
                "tools/call" => {
                    ...
                    let Ok(screening_type) = ScreeningType::parse(
                        args.get("screening_type")
                            .and_then(Value::as_str)
                            .unwrap_or(""),
                    ) else {
                        return JsonRpcResponse::err(id, -32602, "bad type");
                    };
                    let responses: Vec<u8> = args
                        .get("responses")
                ...
                "resources/read" => match self.s.list_t.execute(50).await {
